...
## Task 2: 后端 — 更新路由和依赖注入
- [x] 修改 `internal/router/bot_router.go`：注册 `POST /api/bot/import`
- [x] 修改 `cmd/server.go`：`NewBotHandler` 调用增加 `d` 参数
- [x] `go build ./...` 编译通过

## Task 3: 文档 — 更新 API 文档
- [x] 更新 `docs/api.md` Bot API 部分新增 `POST /api/bot/import` 接口说明
- [x] 更新 `api/openapi.yaml` 新增接口定义
